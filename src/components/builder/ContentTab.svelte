...
<div class="p-4 space-y-4 text-xs">
  ...
  {#if section.type === 'header_announcement'}
    <div class="space-y-3">
      <div>
        <label for="announcement-text" class="block font-medium text-slate-300 mb-1">Teks Pengumuman</label>
        <input
      ...
      <div>
        <span class="block font-medium text-slate-300 mb-1">Menu Navigasi (Reorderable)</span>
        <div class="space-y-2">
          {#each section.props?.navLinks || [] as link, index}
            <div class="flex items-center gap-1.5 p-1 bg-slate-950/60 border border-slate-800 rounded-lg">
              <input
                type="text"
                value={link}
                on:input={(e) => {
                ...
                class="flex-1 px-2.5 py-1.5 bg-slate-950 border border-slate-700 rounded text-slate-100 placeholder-slate-500 focus:outline-none focus:border-blue-500"
              ...
              <div class="flex items-center">
                <button
                  ...
                  disabled={index === 0}
                  class="p-1 text-slate-400 hover:text-slate-200 disabled:opacity-20 cursor-pointer disabled:cursor-not-allowed"
                ...
                <button
                  ...
                  class="p-1 text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 rounded transition-colors cursor-pointer ml-0.5"
                  title="Hapus Link"
                >
                  <Trash2 size={13} />
                </button>
          ...
          <button
            ...
            class="w-full flex items-center justify-center gap-1 py-1.5 border border-dashed border-slate-700 rounded-md text-slate-400 hover:text-blue-400 hover:border-blue-500 transition-colors cursor-pointer"
          >
            <Plus size={13} />
  ...
  {:else if section.type === 'hero'}
    <div class="space-y-3">
      <div>
        <label for="hero-title" class="block font-medium text-slate-300 mb-1">Judul Utama (Title)</label>
        <input
          id="hero-title"
          type="text"
      ...
      <div>
        <label for="hero-subtitle" class="block font-medium text-slate-300 mb-1">Subjudul (Subtitle)</label>
        <textarea
          id="hero-subtitle"
          value={section.props?.subtitle ?? ''}
          on:input={(e) => handlePropChange('subtitle', e.currentTarget.value)}
          rows="3"
          class="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-md text-slate-100 placeholder-slate-500 focus:outline-none focus:border-blue-500 resize-y"
          placeholder="Produk berkualitas dengan harga terjangkau"
        />
      </div>

      <div>
        <label for="hero-image" class="block font-medium text-slate-300 mb-1">URL Gambar Banner</label>
        <input
          id="hero-image"
          type="text"
      ...
      <div class="grid grid-cols-2 gap-2">
        <div>
          <label for="cta-text" class="block font-medium text-slate-300 mb-1">Teks Tombol CTA</label>
          <input
            id="cta-text"
            type="text"
        ...
        <div>
          <label for="cta-link" class="block font-medium text-slate-300 mb-1">Link CTA</label>
          <input
            id="cta-link"
            type="text"
      ...
      <div>
        <label for="tag-name" class="block font-medium text-slate-300 mb-1">HTML Tag Heading</label>
        <select
          id="tag-name"
  ...
  {:else if section.type === 'features'}
    <div class="space-y-3">
      <div class="flex items-center justify-between">
        <span class="block font-medium text-slate-300">Daftar Fitur / Keunggulan</span>
        <button
          type="button"
          on:click={() => handleAddArrayItem('features', { icon: '⭐', title: 'Fitur Baru', description: 'Deskripsi keunggulan produk/layanan Anda.' })}
          class="flex items-center gap-1 text-[11px] font-medium text-blue-400 hover:text-blue-300 cursor-pointer"
        >
          <Plus size={12} />
          ...
          <div class="p-3 bg-slate-950/60 border border-slate-800 rounded-lg space-y-2">
            <div class="flex items-center justify-between gap-2">
              <input
                ...
                class="w-16 px-2 py-1 bg-slate-950 border border-slate-700 rounded text-center text-slate-100 text-sm focus:outline-none focus:border-blue-500"
              ...
              <input
                ...
                class="flex-1 px-2.5 py-1 bg-slate-950 border border-slate-700 rounded text-slate-100 focus:outline-none focus:border-blue-500"
              ...
              <div class="flex items-center">
                ...
                <button
                  ...
                  class="p-1 text-slate-400 hover:text-rose-400 rounded transition-colors cursor-pointer"
                  title="Hapus Fitur"
                >
                  <Trash2 size={13} />
                </button>
            ...
            <textarea
              ...
              rows="2"
              class="w-full px-2.5 py-1 bg-slate-950 border border-slate-700 rounded text-slate-100 focus:outline-none focus:border-blue-500"
  ...
  {:else if section.type === 'product_catalog'}
    <div class="space-y-3">
      <div class="flex items-center justify-between">
        <span class="block font-medium text-slate-300">Preview Katalog Produk</span>
        <button
          type="button"
      ...
      {#if (section.props?.products || []).length === 0}
        <div class="p-4 text-center border border-dashed border-slate-800 rounded-lg text-slate-500">
          <p>Belum ada produk preview.</p>
          <button
            type="button"
            on:click={() => handleAddArrayItem('products', { name: 'Produk Contoh', price: 75000, imageUrl: '', badge: 'Populer' })}
            class="mt-2 text-xs text-blue-400 hover:underline inline-block cursor-pointer"
          >
            + Buat Produk Contoh
          </button>
        </div>
            ...
            <div class="p-3 bg-slate-950/60 border border-slate-800 rounded-lg space-y-2">
              ...
              <div class="grid grid-cols-2 gap-2">
                <input
                  ...
                  class="w-full px-2 py-1 bg-slate-950 border border-slate-700 rounded text-slate-100 focus:outline-none focus:border-blue-500"
  ...
  {:else if section.type === 'testimonials'}
    <div class="space-y-3">
      <div class="flex items-center justify-between">
        <span class="block font-medium text-slate-300">Daftar Testimoni</span>
        <button
          type="button"
          ...
          <div class="p-3 bg-slate-950/60 border border-slate-800 rounded-lg space-y-2">
            <div class="flex items-center justify-between gap-2">
              ...
              <select
                ...
                class="w-16 px-2 py-1 bg-slate-950 border border-slate-700 rounded text-slate-100 text-xs focus:outline-none focus:border-blue-500"
              >
  ...
  {:else if section.type === 'faq'}
    <div class="space-y-3">
      <div class="flex items-center justify-between">
        <span class="block font-medium text-slate-300">Daftar FAQ (Tanya Jawab)</span>
        <button
          type="button"
  ...
  {:else if section.type === 'footer'}
    <div class="space-y-3">
      <div>
        <label for="whatsapp-number" class="block font-medium text-slate-300 mb-1">Nomor WhatsApp Toko</label>
        <input
          id="whatsapp-number"
          type="text"
          value={section.props?.whatsappNumber ?? ''}
          on:input={(e) => handlePropChange('whatsappNumber', e.currentTarget.value)}
          class="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-md text-slate-100 placeholder-slate-500 focus:outline-none focus:border-blue-500 font-mono"
          placeholder="628123456789"
        />
        <p class="text-[11px] text-slate-500 mt-1">Gunakan format internasional tanpa simbol (contoh: 628123456789).</p>
      </div>

      <div>
        <label for="store-address" class="block font-medium text-slate-300 mb-1">Alamat Fisik / Lokasi Toko</label>
        <textarea
          id="store-address"
      ...
      <div>
        <label for="copyright-text" class="block font-medium text-slate-300 mb-1">Teks Hak Cipta (Copyright)</label>
        <input
          id="copyright-text"
          type="text"
