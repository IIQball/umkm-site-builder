<script lang="ts">
  ...
  export let floatingCtaTitle: string = 'Ingin Pesan Menu Katering Hari Ini?';
  export let floatingCtaSubtitle: string = 'Dapatkan diskon ongkir untuk pemesanan minimal 10 porsi.';
...
<div class="relative pt-6">
  ...
  <div class="relative z-10 -mb-8">
    ...
    >
      <div class="text-left space-y-1">
        ...
        <p
          style="font-family: var(--theme-font-body, var(--font-family, inherit)); font-size: var(--theme-text-body, var(--text-body-size, 14px));"
          class="opacity-90"
        >
          {floatingCtaSubtitle}
        </p>
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        style="border-radius: var(--theme-btn-radius, var(--btn-radius, 16px)); background-color: var(--theme-surface, var(--color-card-base)); color: var(--theme-primary, var(--color-primary)); font-family: var(--theme-font-heading, var(--font-heading, inherit)); font-size: var(--theme-text-caption, var(--text-caption-size, 12px));"
        class="h-10 px-6 font-bold hover:opacity-90 shrink-0 inline-flex items-center justify-center gap-1.5 shadow-sm active:scale-[0.98] transition-all"
        on:click|stopPropagation
      >
        <span>{floatingCtaButtonText}</span>
        <ArrowRight size={14} />
      </a>
    </div>
  </div>
  ...
  >
    <div class="cq-footer-bottom-bar">
      ...
      >
        <span
          style="font-family: var(--theme-font-heading, var(--font-heading, inherit)); font-size: var(--theme-text-caption, var(--text-caption-size, 14px)); font-weight: 700; color: var(--theme-text-primary, var(--color-text-main));"
          class="block"
        >
          {brandName}
        </span>
