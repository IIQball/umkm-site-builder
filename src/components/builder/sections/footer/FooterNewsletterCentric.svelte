  ...
  >
    {#if newsletterBadge}
      <span
        ...
        class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[var(--theme-primary, var(--color-primary))]/10 text-[var(--theme-primary, var(--color-primary))]"
      >
        <Sparkles size={12} />
    ...
    <form on:submit|preventDefault={handleSubmit} class="flex flex-col sm:flex-row gap-2 pt-2 max-w-md mx-auto">
      <input
        ...
        style="border-radius: var(--theme-btn-radius, var(--btn-radius, 16px)); background-color: var(--theme-surface, var(--color-card-base)); color: var(--theme-text-primary, var(--color-text-main)); font-family: var(--theme-font-body, var(--font-family, inherit)); font-size: var(--theme-text-body, var(--text-body-size, 14px));"
        class="flex-1 h-10 px-4 border border-[var(--color-border)] outline-none focus:border-[var(--theme-primary, var(--color-primary))] focus:ring-1 focus:ring-[var(--theme-primary, var(--color-primary))] transition-all"
        required
      />
      <button
        type="submit"
