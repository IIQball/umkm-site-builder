...
<div class="max-w-3xl mx-auto py-12 flex flex-col items-center text-center gap-6">
  ...
  <div
    ...
    class={`w-full max-w-md flex flex-col sm:flex-row items-center gap-2 p-2 rounded-2xl bg-[var(--color-card-base)] border border-[var(--color-border)] shadow-md transition-all cursor-pointer ${
      isFormActive
  ...
  >
    <div class="flex items-center gap-2 px-3 flex-1 w-full text-slate-400">
      ...
      <input
        ...
        class="w-full bg-transparent text-xs text-[var(--color-text-main)] focus:outline-none placeholder:text-slate-400"
      />
    </div>
    <a
      href={ctaLink || '#'}
      class="w-full sm:w-auto h-10 min-h-[40px] px-6 rounded-2xl bg-[var(--color-primary)] text-white font-heading font-semibold text-xs inline-flex items-center justify-center hover:bg-primary-dark active:scale-[0.98] transition-all shrink-0 shadow-xs"
    >
