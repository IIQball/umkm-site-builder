...
<div class={`space-y-4 ${align === 'center' ? 'text-center max-w-3xl mx-auto' : 'text-left'}`}>
    ...
    <div
      ...
      class={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-2xs font-heading font-medium border transition-all cursor-pointer shadow-2xs ${
        isBadgeActive
          ...
          : 'bg-slate-100/90 dark:bg-slate-800/70 border-slate-200/90 dark:border-slate-700/80 text-[var(--color-text-secondary)] hover:outline-dashed hover:outline-1 hover:outline-primary/50'
      }`}
    >
  ...
  <svelte:element
    ...
    class={`text-heading-xl font-heading font-extrabold text-[var(--color-text-main)] tracking-tight leading-tight transition-all cursor-pointer rounded-xl p-1.5 -ml-1.5 block ${
      isTitleActive
  ...
  {#if subtitle}
    <div
      ...
      class={`text-body-base text-[var(--color-text-secondary)] leading-relaxed font-sans transition-all cursor-pointer rounded-xl p-1.5 -ml-1.5 ${
        isSubtitleActive
          ? 'ring-2 ring-primary ring-offset-2 dark:ring-offset-slate-900 bg-primary/10'
          : 'hover:outline-dashed hover:outline-1 hover:outline-primary/50'
      }`}
    ...
    >
      {#if ctaText}
        <a
          ...
          class="inline-flex items-center justify-center h-10 min-h-[40px] px-6 rounded-2xl bg-[var(--color-primary)] text-white font-heading font-semibold text-sm hover:bg-primary-dark active:scale-[0.98] transition-all duration-150 shadow-xs"
        >
          <span>{ctaText}</span>
      ...
      {#if secondaryCtaText}
        <a
          ...
          class="inline-flex items-center justify-center h-10 min-h-[40px] px-5 rounded-2xl bg-transparent text-[var(--color-text-main)] hover:bg-[var(--color-nested-base)] border border-slate-300 dark:border-slate-700 font-heading font-semibold text-sm active:scale-[0.98] transition-all duration-150"
        >
