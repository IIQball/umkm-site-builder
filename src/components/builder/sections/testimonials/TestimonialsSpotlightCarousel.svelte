...
{#if activePreset === 'spotlight_hero'}
  <div class="max-w-4xl mx-auto flex flex-col md:flex-row items-center gap-8 bg-[var(--theme-surface, var(--color-card-base))] border border-base-200 dark:border-slate-800 p-8 rounded-3xl shadow-sm text-left">
    <div class="w-32 h-32 sm:w-40 sm:h-40 rounded-full overflow-hidden shrink-0 ring-4 ring-[var(--theme-primary, var(--color-primary))]/20 shadow-md">
      <img
    ...
    <div class="flex-1 flex flex-col gap-3">
      ...
      <p class="text-base sm:text-lg font-medium text-[var(--theme-text-primary, var(--color-text-main))] italic leading-relaxed">
        "{spotlightItem?.comment}"
      </p>
      <div class="flex items-center gap-2 pt-2 border-t border-base-200 dark:border-slate-800">
        <span class="font-bold text-sm text-[var(--theme-text-primary, var(--color-text-main))]">{spotlightItem?.customerName}</span>
        ...
        <div class="flex gap-2 pt-2">
          {#each testimonials as _, i}
            <button
              ...
              class="w-2.5 h-2.5 rounded-full transition-all cursor-pointer {activeSpotlightIdx === i ? 'bg-[var(--theme-primary, var(--color-primary))] w-6' : 'bg-base-300 dark:bg-slate-700'}"></button>
          {/each}
        </div>
      {/if}
    </div>
...
{:else if activePreset === 'carousel_slider'}
  <div class="max-w-2xl mx-auto relative bg-[var(--theme-surface, var(--color-card-base))] border border-base-200 dark:border-slate-800 p-8 rounded-3xl shadow-sm text-center">
    <div class="w-16 h-16 rounded-full overflow-hidden mx-auto mb-4 ring-2 ring-[var(--theme-primary, var(--color-primary))]/30">
      <img
    ...
    <p class="text-base font-medium text-[var(--theme-text-primary, var(--color-text-main))] italic mb-4">
      "{carouselItem?.comment}"
    </p>
    <p class="font-bold text-xs text-[var(--theme-text-primary, var(--color-text-main))]">{carouselItem?.customerName}</p>
