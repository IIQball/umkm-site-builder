...
<div class="relative nav-dropdown-container shrink-0">
  <button
    ...
    class={isActive
      ? 'px-3.5 lg:px-4 py-2 rounded-full bg-main text-canvas dark:bg-white dark:text-slate-950 font-bold text-[11px] lg:text-xs tracking-wider uppercase shadow-sm transition-all duration-200 flex items-center gap-1.5 cursor-pointer shrink-0'
  ...
  {#if isOpen}
    <div
      class="absolute left-0 top-full mt-2.5 w-72 sm:w-80 rounded-2xl bg-card/95 dark:bg-[#11141c]/95 backdrop-blur-xl border border-border/80 dark:border-white/15 text-main shadow-2xl overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-150"
    >
      <div class="max-h-[380px] overflow-y-auto no-scrollbar">
        {#each items as item}
          ...
          >
            ...
            {#if item.desc}
              <p class="text-[11px] sm:text-xs text-secondary dark:text-neutral-400 mt-1 leading-relaxed">
                {item.desc}
              </p>
            {/if}
          </a>
