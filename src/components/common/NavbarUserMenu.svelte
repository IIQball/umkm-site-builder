...
<div class="relative shrink-0" bind:this={menuContainer}>
  ...
  >
    <div class="w-7 h-7 rounded-full bg-orange text-white font-heading font-bold text-xs flex items-center justify-center overflow-hidden shrink-0">
      {#if user.image}
  ...
  {#if isOpen}
    <div class="absolute right-0 mt-2 w-56 rounded-2xl bg-card border border-border text-main shadow-2xl p-2 space-y-1 z-50">
      <div class="px-3 py-2 border-b border-border dark:border-white/10 flex items-center justify-between">
        ...
        {#if roleMeta}
          <span class="label-caps text-xs-dense px-2 py-0.5 rounded border {roleMeta.color}">
            {roleMeta.label}
          </span>
        {/if}
      </div>
      <div class="py-1">
        {#each roleNavLinks as link}
          <a href={link.href} class="flex items-center gap-2 px-3 py-1.5 rounded-lg body-sm text-secondary hover:text-main hover:bg-nested dark:text-slate-300 dark:hover:text-white dark:hover:bg-white/10 transition-colors">
      ...
      <div class="border-t border-border dark:border-white/10 pt-1">
        <button
          ...
          class="w-full flex items-center gap-2 px-3 py-1.5 rounded-lg text-rose-600 dark:text-rose-400 hover:bg-rose-500/10 body-sm font-medium transition-colors cursor-pointer"
        >
          <LogOut size={14} />
          <span>Keluar</span>
        </button>
