      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
        {#each filteredProducts as product (product.id)}
          <div class="group bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
            ...
            <!-- Content -->
            <div class="p-4">
              <h3 class="font-semibold text-slate-900 line-clamp-1 mb-1" title={product.name}>{product.name}</h3>
              <p class="text-primary font-bold">{formatCurrency(product.basePrice)}</p>
              
              <button class="w-full mt-4 py-2 px-4 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2">
